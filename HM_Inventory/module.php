...
class HMInventoryReportCreator extends IPSModule {
    ...
    const BG_COLOR_GLOBAL           = '#181818';         // Global background color
    const BG_COLOR_INTERFACE_LIST   = '#223344';         // Background color for the interface list
    const BG_COLOR_HEADLINE         = '#334455';         // Background color for the header line of the device list
    ...
    public function CreateReport() {
        ...
        foreach (IPS_GetInstanceListByModuleID('{EE4A81C6-5C90-4DB7-AD2F-F6BBD521412E}') as $id) {
            ...
            $HM_array[] = ['IPS_occ'             => $HM_module_num,
                           'IPS_id'              => $id,
                           'IPS_name'            => $IPS_name,
        ...
        foreach ($hm_dev_list as $hm_dev) {
            ...
            if ($hm_dev_in_array == false) {
                if (isset($hm_dev['PARENT']) && ($hm_dev['PARENT'] != '')) {
        ...
        */
        ...
        if ($xml_rtnmsg->errno == 0) {
            ...
            print "Warning: Can't get RF-level information from the BidCos-Service ($BidCos_RF_Service_adr) &nbsp&nbsp&nbsp - &nbsp&nbsp&nbsp ($xml_rtnmsg->errstr)<br>\n";
        }
        ...
        foreach ($HM_array as &$HM_dev){
            ...
            if ($hm_adr[0] != $previous_hm_adr){
                $xml_method = new xmlrpcmsg('getParamset',
                ...
                if ($xml_rtnmsg->errno == 0){
                    $HM_ParamSet = php_xmlrpc_decode($xml_rtnmsg->value());
                    //            print_r($HM_ParamSet);
                    if (isset($HM_ParamSet['RSSI_PEER'])){
                        $HM_dev['HM_levels'][$HM_default_interface_no][0] = $HM_ParamSet['RSSI_PEER'] ;
                    } else {
                        $HM_dev['HM_levels'][$HM_default_interface_no][0] = 65536 ;
                    }
                    if (isset($HM_ParamSet['RSSI_DEVICE'])){
                        $HM_dev['HM_levels'][$HM_default_interface_no][1] = $HM_ParamSet['RSSI_DEVICE'] ;
                    } else {
        ...
        $HTML_ifcs .= "<td><table align='left'><tr><td><font size='3' color='#99AABB'><b>HM Inventory (".$this::VERSION.") </font></b>";
        ...
        $HTML_ifcs .= "<td width='40%' valign='bottom'><table width='100%' align='right' bgcolor=".$this::BG_COLOR_INTERFACE_LIST.'>';
        //	print_r($hm_BidCos_Ifc_list);
        foreach ($hm_BidCos_Ifc_list as $hm_ifce) {
        ...
        $HTML_dvcs .= '<tr bgcolor='.$this::BG_COLOR_HEADLINE.'>';
        ...
        $HTML_dvcs .= $dthdr_td_b.'HM device type'.$dthdr_td_eb.'Fw.'.$dthdr_td_eb.'HM channel type'.$dthdr_td_eb.'Dir.'.$dthdr_td_eb.'AES'.$dthdr_td_e;
        ...
        foreach ($HM_array as $HM_dev) {
            ...
            if (($entry_no++ % 2) == 0){
                $r_bgcolor = $this::BG_COLOR_ODDLINE;
            } else {
                $r_bgcolor = $this::BG_COLOR_EVENLINE;
            }
            ...
            $HTML_dvcs .= $HM_dev['HM_devtype'].$dtdvc_td_eb.$HM_dev['HM_direction'].$dtdvc_td_e.$dtdvc_td_ac_b.$HM_dev['HM_AES_active'].$dtdvc_td_e.$dtdvc_td_ac_b;
            ...
            if (!$same_device){
                ...
                for ($lci = 0; $lci < $HM_interface_connected_num; $lci++) {
                    if (isset($HM_dev['HM_levels'][$lci])) {
                        ...
                        $ifce_dev_lvls = $HM_dev['HM_levels'][$lci];
                        ...
                        if ($ifce_dev_lvls[3]) {
                            if (($HM_dev['HM_Roaming'] == '+') || $ifce_dev_lvls[2]) {
                        ...
                        if ($rx_lvl != 65536) {
                            $rx_strg = (string) $rx_lvl;
                        } else {
                            $rx_strg = '--';
                        }
                        if ($tx_lvl != 65536) {
                            $tx_strg = (string) $tx_lvl;
                        } else {
                            $tx_strg = '--';
                        }
                        $lvl_strg = sprintf($fmt_strg, $lvl_strg_color, $rx_strg,
                            $tx_strg);
...
//        $this->RegisterPropertyBoolean('RequestStatusForLevelUpdate', false);
    ...
    static function usort_HM_address($a, $b){
     ...
     static function usort_IPS_dev_name($a, $b){
    ...
    static function usort_HM_device_adr($a, $b){
    ...
    static function usort_HM_type($a, $b){
